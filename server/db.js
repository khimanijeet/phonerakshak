...
// ----------- Support Tickets -----------
function createSupportTicket(phone) {
  const key = normalizePhone(phone);
  if (!key) return null;
  if (!db.supportTickets) db.supportTickets = {};
  if (!db.supportTickets[key]) {
    db.supportTickets[key] = {
      id: uid('tkt'),
      phone: key,
      messages: [],
      priority: 'normal',
  ...
  return db.supportTickets[key];
}

function getSupportTicket(phone) {
  const key = normalizePhone(phone);
  if (!db.supportTickets) return null;
  return db.supportTickets[key] || null;
}

function addSupportMessage(phone, text, isBot) {
  let tkt = getSupportTicket(phone) || createSupportTicket(phone);
  if (!tkt) return null;
...
function escalateTicket(phone, isPremium) {
  let tkt = getSupportTicket(phone);
  if (!tkt) return null;
  tkt.status = 'escalated';
  tkt.priority = isPremium ? 'high' : 'normal';
  tkt.updatedAt = Date.now();
  persist();
  return tkt;
}
