...
class Prefs(context: Context) {
    ...
    /**
     * User consent for the post-boot recovery report. When ON, the app will
     * send a one-shot status update (battery, network, last known location)
     * to the registered backend / emergency contact each time the phone
     * boots. Defaults to OFF — must be explicitly enabled in Setup.
     */
    var bootRecoveryEnabled: Boolean
        get() = prefs.getBoolean(KEY_BOOT_RECOVERY, false)
        set(value) = prefs.edit().putBoolean(KEY_BOOT_RECOVERY, value).apply()

    /**
     * Last connected Wi-Fi snapshot as a JSON string (SSID, BSSID, signal,
     * link speed, frequency, timestamp). Useful for recovery if GPS is off
     * or indoors — the BSSID identifies the exact router.
     */
    var lastKnownWifi: String?
        get() = prefs.getString(KEY_LAST_WIFI, null)
        set(value) = prefs.edit().putString(KEY_LAST_WIFI, value).apply()
    ...
    companion object {
        ...
        private const val KEY_BOOT_RECOVERY = "boot_recovery_enabled"
        private const val KEY_LAST_WIFI = "last_known_wifi"
    }
}
